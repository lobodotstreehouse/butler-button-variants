/* ─────────────────────────────────────────────────────────────────
   ...
   modal that calls the Veltm `butler-booking-api` Supabase Edge
   Function and redirects the browser to the returned Stripe
   Checkout URL. Zoho Forms + Stripe Payment Links are no longer
   used; the Edge Function handles DB insert, pricing, Checkout
   session, welcome email, voucher, Zoho Desk/CRM, ZeptoMail.
   ...
   Endpoint:
     POST {SUPABASE_URL}/functions/v1/butler-booking-api
   Auth (browser-safe):
     apikey:        <Supabase anon key>     // public, ships in HTML
     Authorization: Bearer <Supabase anon key>
   The function is locked down server-side via CORS allow-list
   (butlerbutton.co + www) and RLS — no shared secret is required
   from the browser. The previous `x-api-key` (BUTLER_BOOKING_API_KEY)
   is intentionally NOT sent here and must not be embedded in client
   code.

   Tier mapping (modal -> API):
     trip -> trip_planning
     8h   -> 8hrs
     24h  -> 24hrs
   ───────────────────────────────────────────────────────────────── */
